...
| User says "modify the XXX diagram" (no file provided, by name) | **Find by metadata:** (1) `grep -rl "%% title:" .drawio/type/*.mmd` to list all Mermaid sources with titles, (2) `grep -l "关键词" .drawio/type/*.mmd` for keyword match on `%% keywords:` lines, (3) Fuzzy-match user's description against `%% title:` and `%% keywords:` fields. If `.mmd` found → edit source → regenerate. If only `.drawio` found → parse and edit. Every Pipeline B `.mmd` must have `%% title:`, `%% type:`, `%% keywords:` comment headers (see `references/mermaid-authoring.md`). |
