...
Exit codes:
    ...
    1 — P1/P2 errors found (or any warning with --strict)
...
def validate_drawio(filepath):
    ...
    # ── P0: R002 — Duplicate / Reserved IDs ──
    ids_seen = defaultdict(list)
    for cell_id, cell in all_cells.items():
        ids_seen[cell_id].append(cell)

    for cell_id, cells in ids_seen.items():
        if len(cells) > 1:
            errors.append({
                "id": "R002",
                "message": f"Duplicate cell id='{cell_id}' found {len(cells)} times"
            })

    # Check for reserved ID usage (id=0 or id=1 for user cells)
    for cell in all_cells.values():
        cell_id = cell.get('id', '')
        parent = cell.get('parent', '')
        if cell_id in RESERVED_IDS:
            # id=0 and id=1 are the root cells — they should exist
            # but other cells should not USE 0 or 1 as their own id
            pass  # Root cells with id 0/1 are expected

    # Check: any user cell (id >= 2) that has id "0" or "1"?
    # (Root cells are expected; user cells should not use reserved ids)
    ...
    for edge in all_edges:
        ...
        if source_id in all_cells:
            src_geom = parse_geometry(all_cells[source_id])
        if target_id in all_cells:
            tgt_geom = parse_geometry(all_cells[target_id])
        ...
        # Reconstruct full path: source_center -> waypoints -> target_center
        path = []
        if src_geom:
            sx, sy, sw, sh = src_geom
            path.append((sx + sw / 2, sy + sh / 2))
        else:
            if points:
    ...
    for cell in list(all_vertices) + list(all_edges):
        ...
        if geom:
            x, y, w, h = geom
            off_fields = []
            if x != 0 and is_off_grid(x):
                off_fields.append(f"x={x}")
            if y != 0 and is_off_grid(y):
